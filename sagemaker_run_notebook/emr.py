...
def get_cluster_info(cluster_name, session=None):
    ...
    while True:
        if marker:
            ...
            marker_args = {}
        list_results = emr.list_clusters(ClusterStates=['STARTING', 'BOOTSTRAPPING', 'RUNNING', 'WAITING'], **marker_args)
        marker = list_results.get("Marker")
        cluster_info = list(it.islice(filter(lambda c: c.get('Name') == cluster_name, list_results["Clusters"]), 1))
        if cluster_info:
...
def add_emr_cluster(cluster):
    ...
    def proc(extra):
        ...
        if net.get("VpcConfig"):
            raise RuntimeError("Can't add EMR cluster if there's already a VPC connection specified")
